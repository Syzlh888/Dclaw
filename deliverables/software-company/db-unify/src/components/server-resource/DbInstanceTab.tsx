...
export default function DbInstanceTab({ serverId, instances, ports, serverIps = [] }: Props) {
  ...
  return (
    <Box>
      ...
      {instances.length === 0 ? (
        <Typography variant="body2" color="text.secondary" sx={{ pt: 0.5, textAlign: 'left', fontSize: 'calc(0.8rem * var(--dc-scale, 1))' }}>暂无数据库实例</Typography>
      ) : (
        <TableContainer>
          <Table size="small">
            <TableHead><TableRow>
              <TableCell sx={{ fontWeight: 600, fontSize: 'calc(0.75rem * var(--dc-scale, 1))' }}>类型</TableCell>
              <TableCell sx={{ fontWeight: 600, fontSize: 'calc(0.75rem * var(--dc-scale, 1))' }}>库名</TableCell>
              <TableCell sx={{ fontWeight: 600, fontSize: 'calc(0.75rem * var(--dc-scale, 1))' }}>IP:端口</TableCell>
              <TableCell sx={{ fontWeight: 600, fontSize: 'calc(0.75rem * var(--dc-scale, 1))' }}>集群</TableCell>
              <TableCell sx={{ fontWeight: 600, fontSize: 'calc(0.75rem * var(--dc-scale, 1))' }}>Schema</TableCell>
              <TableCell sx={{ fontWeight: 600, fontSize: 'calc(0.75rem * var(--dc-scale, 1))' }}>连接名称</TableCell>
            ...
            <TableBody>
              {instances.map(d => {
                ...
                return creds.map((cred, ci) => {
                  ...
                  return (
                    <TableRow key={pwdKey}>
                      {ci === 0 && <TableCell rowSpan={credCount}><Chip label={d.dbType} size="small" sx={{ fontSize: 'calc(0.7rem * var(--dc-scale, 1))' }} /></TableCell>}
                      {ci === 0 && <TableCell rowSpan={credCount} sx={{ fontSize: 'calc(0.8rem * var(--dc-scale, 1))' }}>{d.dbName}</TableCell>}
                      {ci === 0 && <TableCell rowSpan={credCount} sx={{ fontSize: 'calc(0.8rem * var(--dc-scale, 1))' }}>{d.internalIp || d.externalIp || '-'}:{d.port}</TableCell>}
                      {ci === 0 && <TableCell rowSpan={credCount}>
                        {d.isCluster ? <Chip label="集群" size="small" color="warning" sx={{ fontSize: 'calc(0.75rem * var(--dc-scale, 1))' }} title={d.clusterIps || ''} /> : '-'}
                      </TableCell>}
                      <TableCell sx={{ fontSize: 'calc(0.8rem * var(--dc-scale, 1))' }}>{cred.schema || '-'}</TableCell>
                      <TableCell sx={{ fontSize: 'calc(0.8rem * var(--dc-scale, 1))' }}>{cred.connectionName || '-'}</TableCell>
                      <TableCell sx={{ fontSize: 'calc(0.8rem * var(--dc-scale, 1))' }}>{cred.username || '-'}</TableCell>
                      <TableCell sx={{ fontSize: 'calc(0.75rem * var(--dc-scale, 1))' }}>
                      ...
                      {ci === 0 && <TableCell rowSpan={credCount} sx={{ fontSize: 'calc(0.8rem * var(--dc-scale, 1))', maxWidth: 160 }}>{d.notes || '-'}</TableCell>}
                      {ci === 0 && (
                        <TableCell rowSpan={credCount} sx={{ verticalAlign: 'middle', textAlign: 'center' }}>
                          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
                            <Tooltip title="编辑实例">
                              <IconButton size="small" onClick={() => openEdit(d)}><EditIcon sx={{ fontSize: 'calc(1rem * var(--dc-scale, 1))' }} /></IconButton>
                            </Tooltip>
                            <Tooltip title="删除实例">
                              <IconButton size="small" onClick={() => handleDelete(d)} sx={{ color: 'error.main' }}><DeleteIcon sx={{ fontSize: 'calc(1rem * var(--dc-scale, 1))' }} /></IconButton>
                            </Tooltip>
                          </Box>
                        </TableCell>
                      )}
              ...
              {credentials.map((cred, i) => (
                <Box key={i} sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, mb: 1, p: 1.5, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
                  <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center' }}>
                    ...
                    <Button size="small" variant="outlined" onClick={() => setPwdGenDialog(i)} sx={{ minWidth: 56, flexShrink: 0, fontSize: 'calc(0.7rem * var(--dc-scale, 1))', height: 40 }}>生成</Button>
                    <Tooltip title="密码历史">
                      <IconButton
                        size="small"
                        onClick={() => cred.username ? setCredHistoryOpen({ index: i, username: cred.username }) : null}
                        sx={{ flexShrink: 0, height: 40, width: 40 }}
                      ><HistoryIcon sx={{ fontSize: 'calc(1rem * var(--dc-scale, 1))' }} /></IconButton>
                    </Tooltip>
                    {credentials.length > 1 && (
                      <IconButton size="small" onClick={() => removeCred(i)} sx={{ color: 'error.main', flexShrink: 0 }}><DeleteIcon sx={{ fontSize: 'calc(1rem * var(--dc-scale, 1))' }} /></IconButton>
                    )}
                  </Box>
                </Box>
              ))}
