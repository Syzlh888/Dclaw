...
const ServerLocationDictDialog: React.FC<Props> = ({ open, onClose }) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      // 检查是否被服务器引用
      const usage = await checkDictUsage('serverLocation', name);
      if (usage.inUse) {
        const serverNames = usage.serverNames?.join('、') || '';
        setSnackbar({
          open: true,
          message: `无法删除 "${name}"，有 ${usage.count} 台服务器使用此位置${serverNames ? `（如：${serverNames}）` : ''}`,
          severity: 'error',
        });
      } else {
  ...
  return (
    <>
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 600, fontSize: 'calc(1rem * var(--dc-scale, 1))' }}>服务器位置字典</DialogTitle>
        <DialogContent>
                  ...
                  {editingName === item.name ? (
                    <Box sx={{ display: 'flex', gap: 0.5, width: '100%', alignItems: 'center' }}>
                      ...
                      <TextField size="small" value={editShortName} onChange={e => setEditShortName(e.target.value)}
                        onKeyDown={e => { if (e.key === 'Enter') handleSaveEdit(); }}
                        sx={{ flex: 1 }} />
                      <IconButton size="small" onClick={handleSaveEdit}><CheckIcon sx={{ fontSize: 'calc(1rem * var(--dc-scale, 1))' }} color="success" /></IconButton>
                    ...
                    <>
                      ...
                      />
                      <IconButton size="small" onClick={() => startEdit(item)}><EditIcon sx={{ fontSize: 'calc(1rem * var(--dc-scale, 1))' }} /></IconButton>
                      <IconButton size="small" onClick={() => handleDeleteClick(item.name)}><DeleteIcon sx={{ fontSize: 'calc(1rem * var(--dc-scale, 1))' }} /></IconButton>
                    </>
                  )}
                </ListItem>
              ))}
