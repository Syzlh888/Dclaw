  ...
  return (
    <>
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 600, fontSize: 'calc(1rem * var(--dc-scale, 1))' }}>工程字典</DialogTitle>
        <DialogContent>
                  ...
                  {editingId === e.id ? (
                    <Box sx={{ display: 'flex', gap: 0.5, width: '100%', alignItems: 'center' }}>
                      ...
                      <TextField size="small" value={editShortName} onChange={ev => setEditShortName(ev.target.value)}
                        onKeyDown={ev => { if (ev.key === 'Enter') handleSaveEdit(e.id); }}
                        sx={{ flex: 1 }} />
                      <IconButton size="small" onClick={() => handleSaveEdit(e.id)}><CheckIcon sx={{ fontSize: 'calc(1rem * var(--dc-scale, 1))' }} color="success" /></IconButton>
                    ...
                    <>
                      ...
                      />
                      <IconButton size="small" onClick={() => startEdit(e)}><EditIcon sx={{ fontSize: 'calc(1rem * var(--dc-scale, 1))' }} /></IconButton>
                      <IconButton size="small" onClick={() => handleDeleteClick(e.id)}><DeleteIcon sx={{ fontSize: 'calc(1rem * var(--dc-scale, 1))' }} /></IconButton>
                    </>
                  )}
                </ListItem>
              ))}
