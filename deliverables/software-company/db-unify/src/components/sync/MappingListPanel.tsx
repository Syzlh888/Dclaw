...
const IncrementalDialog: React.FC<{ open: boolean; mapping: any; onClose: () => void }> = ({ open, mapping, onClose }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth PaperProps={{ sx: { bgcolor: '#3C3F41', color: 'text.primary' } }}>
      <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, fontSize: 'calc(0.95rem * var(--dc-scale, 1))', py: 1 }}>
        <UpdateIcon sx={{ fontSize: 'calc(1rem * var(--dc-scale, 1))', color: 'primary.main' }} />
        增量同步配置
      </DialogTitle>
      <DialogContent sx={{ pt: '12px !important', py: 1.25 }}>
        <Typography sx={{ color: 'text.secondary', fontSize: 'calc(0.62rem * var(--dc-scale, 1))', mb: 1.5 }}>{mapping?.source_table} → {mapping?.target_table}</Typography>
        <FormControlLabel
        ...
        {enabled && (
          <>
            <TextField
              ...
              sx={{ mb: 1.25, '& .MuiInputBase-root': { color: 'text.primary' } }}
              helperText="源表中用于跟踪增量进度的字段名"
            />
            <Select fullWidth size="small" value={type} onChange={(e) => setType(e.target.value as 'timestamp' | 'numeric')} sx={{ mb: 1.25, color: 'text.primary' }}>
            ...
            <TextField
              ...
              sx={{ mb: 0.75, '& .MuiInputBase-root': { color: 'text.primary' } }}
...
const MappingListPanel: React.FC<Props> = ({ onCreateMapping, onEditColumns }) => {
  ...
  if (selected) return <Box sx={{ p: 1.25, height: '100%', overflow: 'auto' }}>
    <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
      <Typography sx={{ color: 'text.primary', fontWeight: 600 }}>字段映射</Typography>
      <Chip size="small" label={`${selected.column_mappings?.length || 0} 个字段`} sx={{ ml: 1, height: 18, color: 'text.secondary', bgcolor: 'divider' }} />
      <Box sx={{ flex: 1 }} />
      <Button
        size="small"
        startIcon={<AccountTreeIcon sx={{ fontSize: 'calc(0.9rem * var(--dc-scale, 1))' }} />}
    ...
    <Typography sx={{ color: 'text.secondary', fontSize: 'calc(0.7rem * var(--dc-scale, 1))', mb: 1 }}>{selected.source_table} → {selected.target_table}</Typography>
    <Box sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 30px 1fr 100px', p: 0.75, color: 'text.secondary', fontSize: 'calc(0.62rem * var(--dc-scale, 1))' }}><span>源字段</span><span /><span>目标字段</span><span>类型</span></Box><Divider sx={{ borderColor: 'divider' }} />
      {(selected.column_mappings || []).map((column, index) => <Box key={`${column.source}-${index}`} sx={{ display: 'grid', gridTemplateColumns: '1fr 30px 1fr 100px', alignItems: 'center', p: 0.75, color: 'text.secondary', fontSize: 'calc(0.68rem * var(--dc-scale, 1))', borderBottom: index < (selected.column_mappings?.length || 0) - 1 ? '1px solid' : 0, borderColor: 'divider' }}><span>{column.source}</span><span>→</span><span>{column.target}</span><span style={{ color: 'text.secondary' }}>{column.type || '-'}</span></Box>)}
      {(selected.column_mappings || []).length === 0 && <Typography sx={{ p: 2.5, color: 'text.disabled', textAlign: 'center', fontSize: 'calc(0.7rem * var(--dc-scale, 1))' }}>未配置字段映射（默认按同名字段同步）。点击右上「字段映射」配置。</Typography>}
    </Box>
  ...
  return <Box sx={{ p: 1.25, height: '100%', overflow: 'auto' }}>
    <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}><Typography sx={{ color: 'text.primary', fontWeight: 600 }}>表映射</Typography><Chip size="small" label={list.length} sx={{ ml: 1, height: 18, color: 'text.secondary' }} /><Box sx={{ flex: 1 }} /><Button size="small" startIcon={<AddIcon />} onClick={onCreateMapping} sx={{ color: 'primary.main' }}>新建映射</Button></Box>
    {list.length === 0 && <Typography sx={{ mt: 5, textAlign: 'center', color: 'text.disabled', fontSize: 'calc(0.78rem * var(--dc-scale, 1))' }}>该任务暂无表映射</Typography>}
    {list.map((mapping) => {
      const statusMark = mappingStatusMark(mapping.last_run_status);
      const StatusIcon = statusMark.Icon;
      return <Box key={mapping.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, p: 1, mb: 0.75, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: 1, '&:hover': { borderColor: 'primary.main' } }}>
        <Box onClick={() => selectMapping(mapping.id)} sx={{ display: 'flex', alignItems: 'center', gap: 1, flex: 1, minWidth: 0, cursor: 'pointer' }}>
          <TableChartIcon sx={{ color: 'primary.light', fontSize: 'calc(0.9rem * var(--dc-scale, 1))' }} />
          <Box sx={{ flex: 1, minWidth: 0 }}>
            <Typography sx={{ color: 'text.primary', fontSize: 'calc(0.75rem * var(--dc-scale, 1))' }}>{mapping.source_table} → {mapping.target_table}</Typography>
            <Typography sx={{ color: 'text.secondary', fontSize: 'calc(0.62rem * var(--dc-scale, 1))' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.6, mt: 0.4 }}>
              <Tooltip title={statusMark.tip}>
                <StatusIcon sx={{ fontSize: 'calc(0.68rem * var(--dc-scale, 1))', color: statusMark.color }} />
              </Tooltip>
              <Typography sx={{ color: statusMark.color, fontSize: 'calc(0.6rem * var(--dc-scale, 1))' }}>
                {mapping.last_run_status === 'success' ? '成功'
                  : mapping.last_run_status === 'failed' ? '失败'
                  : mapping.last_run_status === 'running' ? '运行中'
                  : '未运行'}
              </Typography>
              <Typography sx={{ color: 'text.disabled', fontSize: 'calc(0.6rem * var(--dc-scale, 1))' }}>·</Typography>
              <Typography sx={{ color: 'text.secondary', fontSize: 'calc(0.6rem * var(--dc-scale, 1))' }} title={mapping.last_run_at || ''}>
                {fmtShortTime(mapping.last_run_at)}
              </Typography>
              {Number(mapping.last_run_rows) > 0 && (
                <>
                  <Typography sx={{ color: 'text.disabled', fontSize: 'calc(0.6rem * var(--dc-scale, 1))' }}>·</Typography>
                  <Typography sx={{ color: 'text.secondary', fontSize: 'calc(0.6rem * var(--dc-scale, 1))' }}>
          ...
          <Chip size="small" label={mapping.enabled === false ? '停用' : '启用'} color={mapping.enabled === false ? 'default' : 'success'} variant="outlined" sx={{ height: 18, fontSize: 'calc(0.6rem * var(--dc-scale, 1))' }} />
        </Box>
        <Tooltip title="字段映射">
          <IconButton
            size="small"
            ...
            sx={{ color: 'primary.main', border: '1px solid', borderColor: 'divider', '&:hover': { borderColor: 'primary.main' }, p: 0.4 }}
          >
            <AccountTreeIcon sx={{ fontSize: 'calc(0.9rem * var(--dc-scale, 1))' }} />
          </IconButton>
        </Tooltip>
        <Tooltip title="增量同步">
          <IconButton
            ...
            sx={{ color: mapping.incremental_column ? 'primary.light' : 'text.secondary', border: '1px solid', borderColor: 'divider', '&:hover': { borderColor: 'primary.main' }, p: 0.4 }}
          >
            <UpdateIcon sx={{ fontSize: 'calc(0.9rem * var(--dc-scale, 1))' }} />
          </IconButton>
        </Tooltip>
      </Box>;
    })}
