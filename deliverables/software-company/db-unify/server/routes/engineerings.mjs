...
router.get('/', async (req, res) => {
  let items = await getAll('engineerings');
...
router.post('/', async (req, res) => {
  ...
  await insert('engineerings', record);
  res.status(201).json(record);
});

router.put('/:id', async (req, res) => {
  const existing = await getById('engineerings', req.params.id);
  ...
  const updated = await update('engineerings', req.params.id, partial);
  res.json(updated);
});
...
router.delete('/:id', async (req, res) => {
  ...
  if (!existing) return res.status(404).json({ error: '工程不存在' });
  // 检查是否被服务器引用
  const serversUsing = await query('servers', s => s.engineering_id === req.params.id);
  ...
  // 检查是否有关联应用
  const childApps = await query('applications', a => a.engineering_id === req.params.id);
  ...
  await remove('engineerings', req.params.id);
