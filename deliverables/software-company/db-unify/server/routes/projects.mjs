...
router.put('/:id', async (req, res) => {
  ...
  const updated = await update('projects', req.params.id, partial);
  res.json(updated);
});
...
router.delete('/:id', async (req, res) => {
  ...
  if (!existing) return res.status(404).json({ error: '项目不存在' });
  // 检查是否被服务器引用
  const serversUsing = await query('servers', s => s.project_id === req.params.id);
  ...
  // 检查是否有关联工程
  const childEngineerings = await query('engineerings', e => e.project_id === req.params.id);
